-- =====================================================
-- BEE RANK DATABASE SCHEMA FOR NEON POSTGRESQL
-- =====================================================
...
CREATE TABLE IF NOT EXISTS transit_routes (
    ...
    rank_id INTEGER REFERENCES taxi_ranks(id) ON DELETE CASCADE,
    origin_rank_id INTEGER REFERENCES taxi_ranks(id) ON DELETE CASCADE,
    destination_rank_id INTEGER REFERENCES taxi_ranks(id) ON DELETE CASCADE,
    ...
    departure_time TIME,
    arrival_time TIME,
    route_type VARCHAR(50) DEFAULT 'taxi' CHECK (route_type IN ('taxi', 'bus', 'mixed', 'walking')),
...
CREATE TABLE IF NOT EXISTS journeys (
                                        id SERIAL PRIMARY KEY,
                                        journey_id UUID UNIQUE NOT NULL DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    origin_rank_id INTEGER REFERENCES taxi_ranks(id),
    destination_rank_id INTEGER REFERENCES taxi_ranks(id),
    ...
    journey_type VARCHAR(50) DEFAULT 'direct' CHECK (journey_type IN ('direct', 'connected', 'no_route_found')),
...
-- Route Connections Table
CREATE TABLE IF NOT EXISTS route_connections (
                                                 id SERIAL PRIMARY KEY,
                                                 journey_id UUID REFERENCES journeys(journey_id) ON DELETE CASCADE,
    route_id INTEGER REFERENCES transit_routes(id) ON DELETE CASCADE,
    sequence_order INTEGER NOT NULL,
    connection_rank_id INTEGER REFERENCES taxi_ranks(id),
    segment_fare DECIMAL(10, 2),
...
CREATE TABLE IF NOT EXISTS hiking_signs (
    ...
    sign_type VARCHAR(50) DEFAULT 'fare_board',
...
CREATE TABLE IF NOT EXISTS user_activities (
    ...
    entity_id INTEGER,
    rank_id INTEGER REFERENCES taxi_ranks(id) ON DELETE SET NULL,
    sign_id INTEGER REFERENCES hiking_signs(id) ON DELETE SET NULL,
    route_id INTEGER REFERENCES transit_routes(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS review_votes (
    ...
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    vote_type VARCHAR(25) CHECK (vote_type IN ('helpful', 'not_helpful')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(review_id, user_id)
    );

-- =====================================================
-- SYSTEM TABLES (No transactions table)
-- =====================================================
